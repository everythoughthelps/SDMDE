...
class bts(nn.Module):
	...
	def forward(self, features, focal,maxdepth):
		maxdepth1 = maxdepth.unsqueeze(1).unsqueeze(1).unsqueeze(1)
		...
		reduc8x8 = self.reduc8x8(torch_nn_func.interpolate(unify_out,size=(daspp_feat.size(2),daspp_feat.size(3))),maxdepth)
		#reduc8x8 = self.reduc8x8(daspp_feat,maxdepth1)
		...
		depth_8x8_scaled = depth_8x8.unsqueeze(1) / maxdepth1
		...
		reduc4x4 = self.reduc4x4(iconv3,maxdepth1)
		...
		depth_4x4_scaled = depth_4x4.unsqueeze(1) / maxdepth1
		...
		reduc2x2 = self.reduc2x2(iconv2,maxdepth1)
		...
		depth_2x2_scaled = depth_2x2.unsqueeze(1) / maxdepth1

		upconv1 = self.upconv1(iconv2)
		reduc1x1 = self.reduc1x1(upconv1,maxdepth1)
		concat1 = torch.cat([upconv1, reduc1x1, depth_2x2_scaled, depth_4x4_scaled, depth_8x8_scaled], dim=1)
		iconv1 = self.conv1(concat1)
		final_depth = maxdepth1 * self.get_depth(iconv1)
		if maxdepth1[0][0][0][0] == 80:
			final_depth = final_depth * focal.view(-1, 1, 1, 1).float() / 715.0873

		return depth_8x8_scaled, depth_4x4_scaled, depth_2x2_scaled, reduc1x1, final_depth,unify_out
